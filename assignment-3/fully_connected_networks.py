...
class ReLU(object):
    ...
    def forward(x):
        ...
        #############################################################################
        # Replace "pass" statement with your code
        out = torch.clamp(x, min=0, max=None)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        cache = x
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ###########################################################################
        # Replace "pass" statement with your code
        hid, cache_hid = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])
        scores, cache_sc = Linear.forward(hid, self.params['W2'], self.params['b2'])
        ###########################################################################
        #                            END OF YOUR CODE                             #
        ###########################################################################
        ...
        dout, grads['W2'], grads['b2'] = Linear.backward(dout, cache_sc)
        grads['W2'] += 2 * self.reg * self.params['W2']

        # propagate through layer 1
        _, grads['W1'], grads['b1'] = Linear_ReLU.backward(dout, cache_hid)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        # iterate from 1 to L inclusive
        for l in range(1, self.num_layers+1):

            if l == 1:
                in_features = input_dim 
                out_features =  hidden_dims[l-1]
            elif l == self.num_layers:
                in_features = hidden_dims[l-2] 
                out_features = num_classes
            else:
                in_features = hidden_dims[l-2]
                out_features = hidden_dims[l-1]

            self.params[f'W{l}'] = weight_scale * torch.randn(
                in_features, out_features, 
                dtype=dtype, device=device
            )
            self.params[f'b{l}'] = torch.zeros(
                out_features, 
                dtype=dtype, device=device
            )
    ...
    def loss(self, X, y=None):
        ...
        for l in range(1, L):
            hidden, cache[f'l{l}'] = Linear_ReLU.forward(
                hidden, self.params[f'W{l}'], self.params[f'b{l}'])

            if self.use_dropout:
                hidden, cache[f'd{l}'] = Dropout.forward(hidden, self.dropout_param)

        # compute scores using layer L
        scores, cache[f'l{L}'] = Linear.forward(
                hidden, self.params[f'W{L}'], self.params[f'b{L}'])
        ...
        for l in range(1, L+1):
            loss += self.reg * torch.sum(self.params[f'W{l}'] * self.params[f'W{l}'])

        # propagate through layer L
        dout, grads[f'W{L}'], grads[f'b{L}'] = Linear.backward(dout, cache[f'l{L}'])
        grads[f'W{L}'] += 2 * self.reg * self.params[f'W{L}']

        # propagate through layers L-1, L-2, ... 1
        for l in reversed(range(1, L)):

            if self.use_dropout:
                dout = Dropout.backward(dout, cache[f'd{l}'])

            dout, grads[f'W{l}'], grads[f'b{l}'] = Linear_ReLU.backward(dout, cache[f'l{l}'])
            grads[f'W{l}'] += 2 * self.reg * self.params[f'W{l}']
...
def get_three_layer_network_params():
    ...
    learning_rate = .01  # Experiment with this!
    # Replace "pass" statement with your code
    pass
    ############################################################################
    #                             END OF YOUR CODE                             #
...
def get_five_layer_network_params():
    ...
    # training accuracy within 20 epochs.                                      #
    ############################################################################
    learning_rate = .0001  # Experiment with this!
    weight_scale = 1   # Experiment with this!
    # Replace "pass" statement with your code
    pass
    ############################################################################
...
def sgd_momentum(w, dw, config=None):
    ...
    ###################################################
    # keep formula as close as possible to cs231n notes
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            # create mask: in cs231n notes p is probability
            # of KEEP and we use '<' sign
