...
def nn_forward_pass(params, X):
        ...
        hidden = torch.clamp(X @ W1 + b1, min=0, max=None)
        scores = hidden @ W2 + b2
...
def nn_forward_backward(params, X, y=None, reg=0.0):
        ...
        # use the code from linear_classifier
        # (N, C)
        ...
        # again reuse the code from previous question
        # compute gradients for the 2nd layer
        # (N, C)
        dscores = probs.clone()
        ...
        # compute gradient of h1 and propagate through ReLU
        # we use here dimensionality to compute grads
        # (N, H) = (N, C) @ (C, H)
        dh1 =  dscores @ W2.T
        dh1[h1 == 0] = 0 

        # compute gradient on W1
        # (D, H) = (D, N) @ (N, H)
        grads['W1'] = X.T @ dh1
...
def nn_get_search_params():
    ...
    # Replace "pass" statement with your code
    hidden_sizes = [1024] 
    regularization_strengths = [1.2*1e-3, 1.1*1e-3]
    learning_rates = [1.47, 1.46, 1.45, 1.44, 1.43]
    learning_rate_decays = [.95]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    ...
    for reg in regularization_strengths:
        for lr in learning_rates:
            for hs in hidden_sizes:
                ...
                # get results and update best model
