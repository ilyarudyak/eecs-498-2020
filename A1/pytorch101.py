...
def multiples_of_ten(start, stop):
    ...
    #############################################################################
    # Replace "pass" statement with your code
    x = torch.arange(start=start//10+1, end=stop//10+1, dtype=torch.float64) * 10
...
def slice_assignment_practice(x):
    ...
    x[:4, :6] = torch.tensor(expected)[:4, :6]
    #############################################################################
    #                            END OF YOUR CODE                               #
    #############################################################################
    return x
...
def shuffle_cols(x):
    ...
    M = x.shape[0]
    rows = torch.arange(M)
    # y = torch.hstack([x[rows, 0:1], x[rows, 0:1], x[rows, 2:3], x[rows, 1:2]])
    y = x[:, [0, 0, 2, 1]]
    #############################################################################
    #                            END OF YOUR CODE                               #
    #############################################################################
...
def make_one_hot(x):
    ...
    # Replace "pass" statement with your code
    x = torch.tensor(x)
    N, M = x.shape[0], torch.max(x) + 1
    y = torch.zeros([N, M], dtype=torch.float32)
...
def zero_row_min(x):
    ...
    #############################################################################
    # Replace "pass" statement with your code
    min_idxs = torch.argmin(x, axis=1)
    x_copy = x.clone()
    x_copy[torch.arange(x.shape[0]), min_idxs] = 0
    y = x_copy
    #############################################################################
    #                            END OF YOUR CODE                               #
    #############################################################################
