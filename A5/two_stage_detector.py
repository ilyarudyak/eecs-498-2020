...
class ProposalModule(nn.Module):
    ...
    def __init__(self, in_dim, hidden_dim=256, num_anchors=9, drop_ratio=0.3, device='cuda'):
        ...
        ##############################################################################
        out_channels = self.num_anchors * 6

        # Make sure that your region proposal module is called pred_layer
        self.pred_layer = nn.Sequential(
            nn.Conv2d(in_channels=in_dim, out_channels=hidden_dim, kernel_size=(3, 3), padding=1),
    ...
    def forward(self, features, pos_anchor_coord=None, \
        ...
        if mode == 'train':  # Training mode.
            ...
            proposals = GenerateProposal(pos_anchor_coord, pos_anchor_offsets, 
                method='FasterRCNN', device=self.device)
